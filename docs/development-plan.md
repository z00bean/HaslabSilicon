...
M6 is active. The command path now executes nodes 0–73 through the third complete C2f block using a reusable static graph IR. It covers SRAM-fit dynamic tiles, 1×1 and 3×3 stride-one/stride-two convolution, repeated bottlenecks, zero-allocation split views, scaled residual adds, common-scale concat materialization, streamed weight/parameter residency, and deterministic lifetime allocation. The 143,156-command schedule reports 12,116,376 DMA bytes and 302,694,400 MACs. Diagnostic execution matches all 3,225,600 compared INT8 values; release execution reduces peak output storage from 2,867,200 to 614,400 bytes and matches the independent 51,200-value final tensor. The next slice is nodes 74–102 through the remaining backbone C2f and SPPF. Independent M3/M4 review remains required before a stable ABI/corpus or package release.
...
| M6 | Minimal compiler and simulated runtime | `ACTIVE` | M5 complete; M3/M4 candidate usable, stable release still gated by independent review | Nodes 0–73 pass in diagnostic and lifetime-reuse modes; completion still requires remaining backbone/neck lowering, INT32 boundary, host tail, and layerwise comparisons |
...
- [ ] Complete the backbone through nodes 74–102: lower the `model.7` downsample, `model.8` fourth C2f, and `model.9` SPPF. Add native MaxPool graph operations and preserve pooled branch lifetimes through concat.
