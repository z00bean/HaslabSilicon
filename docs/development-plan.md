...
M6 is active. The command path now executes nodes 0–47 through the second complete C2f block using a reusable static graph IR. It covers 1×1 and 3×3 stride-one/stride-two convolution, repeated bottlenecks, zero-allocation split views, scaled residual adds, common-scale concat materialization, and deterministic lifetime allocation. The 97,670-command schedule reports 6,951,768 DMA bytes and 194,560,000 MACs. Diagnostic execution matches all 2,764,800 compared INT8 values; release execution reduces peak output storage from 2,457,600 to 614,400 bytes and matches the independent 102,400-value final tensor. The next slice is nodes 48–73 through the `model.5` downsample and `model.6` third C2f. Independent M3/M4 review remains required before a stable ABI/corpus or package release.
...
| M6 | Minimal compiler and simulated runtime | `ACTIVE` | M5 complete; M3/M4 candidate usable, stable release still gated by independent review | Nodes 0–47 pass in diagnostic and lifetime-reuse modes; completion still requires remaining whole-model lowering, INT32 boundary, host tail, and layerwise comparisons |
...
- [ ] Extend the same graph and lifetime allocator through nodes 48–73: the `model.5` downsample and `model.6` third C2f. Preserve diagnostic boundaries, release output rules, exact numerical comparisons, and weight-streaming accounting.
