...
| Throughput versus command traffic | Serialized 128-byte FIFO commands remain a functional candidate | The node 136 schedule already emits 277,436 commands / 35,511,808 command bytes per inference. Complete the graph, measure candidate transports, and choose host refill, one-time linear command fetch, or a revised bounded descriptor mechanism before stable ABI or interface RTL freeze |
...
| Package/preprocessing schema | Sectioned `.hxb`, explicit manifest | Experimental writer/loader exist; freeze required keys, per-section size limits, constants placement, host-tail metadata, malformed-package behavior, and exact image/box semantics before stable compiler/runtime release |
...
The Python golden model and functional command simulator now accompany the numerical and byte-level portions of this contract. The pinned workload, quantization-accuracy evidence, and candidate independent conformance corpus now exist. The experimental M6 package/runtime path executes pinned YOLOv8n nodes 0–136 through the backbone and both top-down neck stages exactly in diagnostic and release allocation modes. Bottom-up neck/head lowering, multi-input and complete command-path accuracy evidence, stable package-schema limits, independent ABI/corpus review, and command-transport resolution remain pre-RTL work. No RTL has been written.

The revision 0.2 review fixes simulator snapshot, extent-reporting, aperture, stride, and epilogue-validation discrepancies and defines the previously ambiguous control rules. M3 remains a freeze candidate until independent command/transition review, M4 conformance evidence, and the M6 command-delivery decision close the gate. M4 fixture work may proceed against this pinned candidate; it must not silently bless simulator output as expected results. Workload, package schema, and physical transport gates remain separate and are assigned in the [review record](reviews/v0-contract-review.md).
