...
| Compiler and runtime | Lower nodes 137–154 through the first bottom-up neck branch | Stride-two `model.16`, concat with the retained `model.12` tensor, non-residual `model.18` C2f lowering, diagnostic versus release allocation, exact boundary comparisons, and `make test` result |
