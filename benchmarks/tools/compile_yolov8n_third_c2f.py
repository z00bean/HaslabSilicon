#!/usr/bin/env python3
"""Compile and validate pinned YOLOv8n nodes 0..73 in two allocation modes."""
...
from haslab_compiler.c2f import load_pinned_through_third_c2f
...
DEFAULT_REPORT = WORKLOAD / "m6-through-third-c2f.json"
...
def main() -> int:
    ...
    stem, first, downsample2, second, downsample3, third, model_hash = (
        load_pinned_through_third_c2f(args.model, args.calibration, args.luts)
    )
    ...
    release_result = exact_result("model.6", release_final, golden["model.6"])
    ...
    for mode, package in packages.items():
        (args.artifact_dir / f"yolov8n-through-third-c2f-{mode}.hxb").write_bytes(package)
    (args.artifact_dir / "yolov8n-through-third-c2f-input.bin").write_bytes(physical_input)
    (args.artifact_dir / "yolov8n-through-third-c2f-diagnostic-output.bin").write_bytes(
        diagnostic_output
    )
    (args.artifact_dir / "yolov8n-through-third-c2f-release-output.bin").write_bytes(
        release_output
    )
    ...
    report = {
        "schema_version": 1,
        "status": "passing_nodes_0_through_73_diagnostic_and_release",
        "scope": {
            "node_indices": [0, 73],
            "nodes": diagnostic_package.manifest["source"]["nodes"],
            "logical_input_shape": [1, 3, 320, 320],
            "compared_boundaries": ordered_names,
            "limitation": "This executes through the third C2f block, not the remaining accelerator graph or host tail.",
        },
