...
M6 is active. The command path now executes nodes 0–102 through the complete backbone and SPPF using a reusable static graph IR. It covers SRAM-fit dynamic tiles, 1×1 and 3×3 stride-one/stride-two convolution, repeated bottlenecks, zero-allocation split views, scaled residual adds, common-scale concat materialization, 5×5 MaxPool with compiled halos, streamed weight/parameter residency, and deterministic lifetime allocation. The 186,928-command schedule reports 16,602,136 DMA bytes and 394,444,800 MACs. Diagnostic execution matches all 3,532,800 compared INT8 values; release execution reduces peak output storage from 3,148,800 to 614,400 bytes and matches the independent 25,600-value backbone tensor. The next slice is nodes 103–119 through the first top-down neck branch. Independent M3/M4 review remains required before a stable ABI/corpus or package release.
...
| M6 | Minimal compiler and simulated runtime | `ACTIVE` | M5 complete; M3/M4 candidate usable, stable release still gated by independent review | Nodes 0–102 pass through the complete backbone in diagnostic and lifetime-reuse modes; completion still requires neck/head lowering, INT32 boundary, host tail, and layerwise comparisons |
...
- [x] Lower the first C2f block with 1×1 convolution, split/view aliases, concat liveness, residual addition, and explicit scale checks. Nodes 0–21 compare exactly across ten boundaries; the split views allocate zero bytes.
- [x] Replace the pinned first-C2f planner with a reusable graph IR and deterministic liveness allocator, then lower nodes 22–47 through the `model.3` downsample and second C2f. Diagnostic mode matches 2,764,800 values across 22 boundaries; release mode saves 1,843,200 output bytes and matches the final tensor.
- [x] Extend the same graph and lifetime allocator through nodes 48–73: the `model.5` downsample and `model.6` third C2f. The scheduler now selects legal 5×5 tiles for 20×20 tensors, streams oversized weights and concat parameters, and passes 3,225,600 diagnostic comparisons plus the release final tensor.
- [x] Complete the backbone through nodes 74–102: lower the `model.7` downsample, `model.8` fourth C2f, and `model.9` SPPF. Native MaxPool uses an explicit -128 halo; release liveness preserves the three pooled branches through concat. Wide convolution and residual parameter records stream within the 3 KiB parameter SRAM.
- [ ] Lower nodes 103–119 through the first top-down neck branch: exact nearest-neighbor `model.10` upsampling, `model.11` concat with the long-lived `model.6` skip tensor, and the non-residual `model.12` C2f block.
...
| 2026-09-23 | Executed nodes 0–21 through the first complete C2f block. The package adds pointwise/stride-one convolutions, zero-allocation split views, explicit scaled residual addition, and common-scale concat materialization. All 1,843,200 values across ten boundaries match exactly. The 59,052-command schedule records 4,368,728 DMA bytes, 86,425,600 MACs, 2,480,256 external bytes, and exact FIFO behavior. | [First-C2f report](../benchmarks/manifests/yolov8n-320-opset13/m6-first-c2f.json), [schedule format](../compiler/first-c2f-format.md), [tests](../compiler/tests/test_c2f.py) |
| 2026-09-23 | Replaced the pinned branch planner with a reusable graph IR and deterministic lifetime allocator, then executed nodes 0–47 through the second C2f. The 97,670-command schedule records 6,951,768 DMA bytes and 194,560,000 MACs. Diagnostic mode matches 2,764,800 values across 22 boundaries; release mode cuts peak output storage from 2,457,600 to 614,400 bytes and exactly matches the 102,400-value final tensor. | [Nodes 0–47 report](../benchmarks/manifests/yolov8n-320-opset13/m6-through-second-c2f.json), [graph format](../compiler/reusable-graph-format.md), [tests](../compiler/tests/test_graph.py) |
| 2026-09-23 | Extended the reusable graph through nodes 0–73 and fixed two scaling limits exposed by the real model: 20×20 tensors now select legal 5×5 SRAM-fit tiles, and a 4,096-byte concat table streams one source at a time through 3 KiB parameter SRAM. The 143,156-command schedule records 12,116,376 DMA bytes and 302,694,400 MACs. Diagnostic mode matches 3,225,600 values across 34 boundaries; release mode saves 2,252,800 output bytes and matches the 51,200-value final tensor. | [Nodes 0–73 report](../benchmarks/manifests/yolov8n-320-opset13/m6-through-third-c2f.json), [graph format](../compiler/reusable-graph-format.md), [tests](../compiler/tests/test_graph.py) |
