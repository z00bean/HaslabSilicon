...
The Python golden model and functional command simulator now accompany the numerical and byte-level portions of this contract. The pinned workload, quantization-accuracy evidence, and candidate independent conformance corpus now exist. An experimental M6 package/runtime slice executes one first-layer tile exactly; full-layer and whole-model lowering, stable package-schema limits, independent ABI/corpus review, and transport decisions remain pre-RTL work. No RTL has been written.
