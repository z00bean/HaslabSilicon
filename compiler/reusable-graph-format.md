...
The M6 reusable scheduler lowers pinned YOLOv8n nodes 0–102 into the experimental `haslab.graph-schedule.v1` package profile. It covers the complete backbone: the two-layer stem, four C2f blocks, three stride-two convolutions, and SPPF. The implementation and schema remain experimental.

## Graph representation

`GraphIR` is an ordered, static-shape graph. Each `GraphTensor` fixes its HWC8 dimensions, binary32 scale, and optional alias relationship. Operations are `ConvSiluOp`, `AddOp`, `ConcatOp`, or `MaxPoolOp`; split is represented by zero-allocation channel views. Validation rejects duplicate tensors, unavailable inputs, invalid HWC8 shapes or scales, duplicate producers, and missing final outputs before commands are emitted.
...
For nodes 0–102, diagnostic output storage is 3,148,800 bytes. Release storage is 614,400 bytes, saving 2,534,400 bytes. Diagnostic and release compilation are separately byte deterministic.
...
The tracked [nodes 0–102 report](../benchmarks/manifests/yolov8n-320-opset13/m6-through-backbone.json) records:

| Measurement | Result |
|---|---:|
| Commands | 186,928 |
| DMA bytes | 16,602,136 |
| INT8 MACs | 394,444,800 |
| `MAXPOOL5_I8` commands | 192 |
| Diagnostic package bytes | 34,599,168 |
| Release package bytes | 34,453,504 |
| Diagnostic peak output bytes | 3,148,800 |
| Release peak output bytes | 614,400 |
| Diagnostic values compared | 3,532,800 |
| Diagnostic mismatches | 0 |
| Release final values compared | 25,600 |
| Release final mismatches | 0 |

Both packages reached the eight-command FIFO high-water mark. Each accepted all 186,928 commands, observed 186,920 full-FIFO responses and refills, and drained eight commands at the end. These are functional submission counts, not cycle, latency, or transport-throughput measurements.

## Remaining work

The next bounded extension is nodes 103–119: exact nearest-neighbor `model.10` upsampling, `model.11` channel concat with the long-lived `model.6` skip tensor, and the non-residual `model.12` C2f block. Whole-model completion also requires the remaining neck branches, INT32 learned-head boundary, and declared host tail. Command compression, transport timing, and hardware resource measurements remain separate work.
