...
Follow-up, 2026-09-25: [M6's second-neck schedule](../../benchmarks/manifests/yolov8n-320-opset13/m6-through-second-neck.json) now executes nodes 0–136 through both top-down neck stages. It reuses the generic upsample/skip/C2f lowering, retains `model.4` from operation 15 through operation 47, and executes `model.15` without a residual add. Diagnostic mode matches 5,145,600 values across 67 boundaries; release mode uses 665,600 output bytes and matches all 102,400 final neck values. Bottom-up neck/head lowering, transport timing, and independent M3/M4 review remain open.
