...
| Compiler and runtime | Extend the pinned YOLOv8n path to the first C2f block: 1×1 convolution, split/view, concat liveness, or residual scales | Node-level validation, exact intermediate comparison, package determinism, and `make test` result |
...
The [development plan](development-plan.md) is the status and dependency record. The [v0 contract](haslab-v0-contract.md), [conformance format](../conformance/FORMAT.md), and [first two-layer schedule](../compiler/multi-layer-format.md) are good starting points. Run `make check`, `make test`, and `git diff --check` for changes that affect executable behavior. Documentation-only changes need `make check` and link/status review.
