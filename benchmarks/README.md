...
The first workload candidate is [YOLOv8n at batch one, 320×320](manifests/yolov8n-320-opset13/README.md). Its exact weight and export are pinned, its FLOAT ONNX graph has been structurally audited against HASLAB v0, and its FLOAT and calibrated signed-symmetric INT8 software baselines are reproducible over all 5,000 COCO val2017 images. The complete first Conv-SiLU layer now compiles and executes through exact HASLAB command semantics; the remaining model, RTL, and hardware measurements remain pending.
