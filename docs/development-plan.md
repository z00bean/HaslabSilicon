...
M6 is active. The command path now executes nodes 0–119 through the complete backbone and first top-down neck C2f using a reusable static graph IR. It covers SRAM-fit dynamic tiles, 1×1 and 3×3 stride-one/stride-two convolution, residual and non-residual C2f blocks, zero-allocation split views, scaled merges, 5×5 MaxPool, exact nearest-neighbor upsampling, long-lived skip tensors, streamed weight/parameter residency, and deterministic lifetime allocation. The 235,474-command schedule reports 20,101,656 DMA bytes and 453,427,200 MACs. Diagnostic execution matches all 4,070,400 compared INT8 values; release execution reduces peak output storage from 3,635,200 to 614,400 bytes and matches the independent 51,200-value neck tensor. The next slice is nodes 120–136 through the second top-down neck branch. Independent M3/M4 review remains required before a stable ABI/corpus or package release.
...
| M6 | Minimal compiler and simulated runtime | `ACTIVE` | M5 complete; M3/M4 candidate usable, stable release still gated by independent review | Nodes 0–119 pass through the first top-down neck C2f in diagnostic and lifetime-reuse modes; completion still requires remaining neck/head lowering, INT32 boundary, host tail, and layerwise comparisons |
...
- [ ] Lower nodes 120–136 through the second top-down neck branch: exact nearest-neighbor `model.13` upsampling, `model.14` concat with the long-lived `model.4` skip tensor, and the non-residual `model.15` C2f block.
