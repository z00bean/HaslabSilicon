...
M6 is active. The command path now executes nodes 0–21 through the first complete C2f block. It covers 1×1 and 3×3 convolution, two zero-allocation split views, a scaled residual add, common-scale concat materialization, and the final pointwise convolution. The package reports 59,052 commands, 4,368,728 DMA bytes, 86,425,600 MACs, and 2,480,256 externally allocated bytes in diagnostic retain-all mode. All 1,843,200 compared INT8 values match the independent golden path, and runtime submission records exact eight-entry FIFO refill behavior. The next slice is a reusable graph IR/liveness scheduler followed by nodes 22–47, including the next downsampling layer and repeated-bottleneck C2f. Independent M3/M4 review remains required before a stable ABI/corpus or package release.
...
| M6 | Minimal compiler and simulated runtime | `ACTIVE` | M5 complete; M3/M4 candidate usable, stable release still gated by independent review | Nodes 0–21 through the first C2f pass exactly; completion still requires reusable whole-model lowering, liveness allocation, host tail, and layerwise comparisons |
...
- [ ] Replace the pinned first-C2f planner with a reusable graph IR and deterministic liveness allocator, then lower nodes 22–47 through the `model.3` downsample and second C2f. Report diagnostic retain-all and release-oriented allocations separately.
