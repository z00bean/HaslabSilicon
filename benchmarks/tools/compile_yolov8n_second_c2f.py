#!/usr/bin/env python3
"""Compile and validate pinned YOLOv8n nodes 0..47 in two allocation modes."""
...
from haslab_compiler.c2f import load_pinned_through_second_c2f
from haslab_ref import add_i8, hwc8_to_nchw, map_i8, nchw_to_hwc8, quantize_int8
...
DEFAULT_REPORT = WORKLOAD / "m6-through-second-c2f.json"
...
def main() -> int:
    ...
    golden["model.3"], logical = integer_conv_silu(logical, downsample)
    ...
    release_result = exact_result("model.4", release_final, golden["model.4"])
    if not release_result["pass"]:
        raise SystemExit("release allocation final output differs from the golden model")
    ...
    output_specs = [
        ...
        (24, [1, 64, 40, 40], "model.3", downsample.output_scale),
    ...
    for mode, package in packages.items():
        (args.artifact_dir / f"yolov8n-through-second-c2f-{mode}.hxb").write_bytes(package)
    (args.artifact_dir / "yolov8n-through-second-c2f-input.bin").write_bytes(physical_input)
    (args.artifact_dir / "yolov8n-through-second-c2f-diagnostic-output.bin").write_bytes(
        diagnostic_output
    )
    (args.artifact_dir / "yolov8n-through-second-c2f-release-output.bin").write_bytes(
        release_output
    )
    ...
    report = {
        "schema_version": 1,
        "status": "passing_nodes_0_through_47_diagnostic_and_release",
        "scope": {
            "node_indices": [0, 47],
            "nodes": diagnostic_package.manifest["source"]["nodes"],
            "logical_input_shape": [1, 3, 320, 320],
            "compared_boundaries": ordered_names,
            "limitation": "This executes through the second C2f block, not the remaining accelerator graph or host tail.",
        },
