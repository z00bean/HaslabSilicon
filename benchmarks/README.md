...
Future FPGA and silicon performance reports should follow [MEASUREMENT.md](MEASUREMENT.md), including the complete host-to-result path, accuracy, and power measurement conditions.

The first workload candidate is [YOLOv8n at batch one, 320×320](manifests/yolov8n-320-opset13/README.md). Its exact weight and export are pinned, its FLOAT ONNX graph has been structurally audited against HASLAB v0, and its FLOAT and calibrated signed-symmetric INT8 software baselines are reproducible over all 5,000 COCO val2017 images. The first two Conv-SiLU blocks now compile into one package and execute through exact HASLAB command semantics with both intermediates retained; the remaining model, RTL, and hardware measurements remain pending.
