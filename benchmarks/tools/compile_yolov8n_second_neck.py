#!/usr/bin/env python3
"""Compile and validate pinned YOLOv8n nodes 0..136 in two allocation modes."""
...
from haslab_compiler.c2f import load_pinned_through_second_neck
...
DEFAULT_REPORT = WORKLOAD / "m6-through-second-neck.json"
...
def main() -> int:
    ...
    (
        ...
        second_neck,
        model_hash,
    ) = load_pinned_through_second_neck(args.model, args.calibration, args.luts)
    ...
    release_result = exact_result("model.15", release_final, golden["model.15"])
    if not release_result["pass"]:
        raise SystemExit("release second-neck output differs from the golden model")
    ...
    for mode, package in packages.items():
        (args.artifact_dir / f"yolov8n-through-second-neck-{mode}.hxb").write_bytes(package)
    (args.artifact_dir / "yolov8n-through-second-neck-input.bin").write_bytes(physical_input)
    (args.artifact_dir / "yolov8n-through-second-neck-diagnostic-output.bin").write_bytes(
        diagnostic_output
    )
    (args.artifact_dir / "yolov8n-through-second-neck-release-output.bin").write_bytes(
        release_output
    )
    ...
    report = {
        "schema_version": 1,
        "status": "passing_nodes_0_through_136_second_neck_diagnostic_and_release",
        "scope": {
            "node_indices": [0, 136],
            "nodes": diagnostic_package.manifest["source"]["nodes"],
            "logical_input_shape": [1, 3, 320, 320],
            "compared_boundaries": ordered_names,
            "limitation": "This executes the backbone and both top-down neck C2f stages, not the bottom-up neck, learned heads, or host tail.",
        },
