#!/usr/bin/env python3
"""Compile and validate pinned YOLOv8n nodes 0..102 in two allocation modes."""
...
from haslab_compiler.c2f import load_pinned_through_backbone
...
DEFAULT_REPORT = WORKLOAD / "m6-through-backbone.json"
...
def main() -> int:
    ...
    (
        ...
        sppf,
        model_hash,
    ) = load_pinned_through_backbone(args.model, args.calibration, args.luts)
    ...
    release_result = exact_result("model.9", release_final, golden["model.9"])
    if not release_result["pass"]:
        raise SystemExit("release backbone output differs from the golden model")
    ...
    for mode, package in packages.items():
        (args.artifact_dir / f"yolov8n-through-backbone-{mode}.hxb").write_bytes(package)
    (args.artifact_dir / "yolov8n-through-backbone-input.bin").write_bytes(physical_input)
    (args.artifact_dir / "yolov8n-through-backbone-diagnostic-output.bin").write_bytes(
        diagnostic_output
    )
    (args.artifact_dir / "yolov8n-through-backbone-release-output.bin").write_bytes(
        release_output
    )
    ...
    report = {
        "schema_version": 1,
        "status": "passing_nodes_0_through_102_backbone_diagnostic_and_release",
        "scope": {
            "node_indices": [0, 102],
            "nodes": diagnostic_package.manifest["source"]["nodes"],
            "logical_input_shape": [1, 3, 320, 320],
            "compared_boundaries": ordered_names,
            "limitation": "This executes the backbone through SPPF, not the neck, learned heads, or host tail.",
        },
