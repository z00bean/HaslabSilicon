...
- For live vision, camera or sensor model, interface, pixel format, resolution, frame rate, exposure/control mode, capture buffers, resize/color path, and dropped-frame policy. State whether the reported latency begins at exposure, sensor arrival, host dequeue, or another reproducible boundary.
...
- Timing: separate capture, preprocessing, transfers, accelerator, and postprocessing; report end-to-end input-to-result latency distributions (at least median, p95, and p99), throughput under stated concurrency, warmup, sample count, timestamp source, clock alignment, and dropped frames.
